    ...
    const validateForm = () => {
        ...
        formData.courts.forEach((c, index) => {
            ...
            if (!c.name.trim()) {
                ...
                ce.name = 'Court name is required'; hasCourtErrors = true;
            }
            if (!c.surface.trim()) {
                if (!firstErrorRef) firstErrorRef = courtsRef;
                ce.surface = 'Surface type is required'; hasCourtErrors = true;
            }
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <Box ref={basicInfoRef}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            <TextField
                                required label="Facility Name" value={formData.name}
                                onChange={(e) => handleInputChange('name', e.target.value)}
                                error={!!errors.name} helperText={errors.name}
                            />
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                <TextField
                                    required label="Sport Type" value={formData.type}
                                    onChange={(e) => handleInputChange('type', e.target.value)}
                                    error={!!errors.type} helperText={errors.type} fullWidth
                                />
                                <TextField
                                    required label="Location Area" value={formData.location}
                    ...
                    <Box ref={contactInfoRef}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            <TextField
                                required label="Facility Address" value={formData.address}
                            ...
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                <TextField
                                    ...
                                    onChange={(e) => handleInputChange('phone', e.target.value)} fullWidth
                                />
                                <TextField
                                    label="Operating Hours" value={formData.hours}
                                    onChange={(e) => handleInputChange('hours', e.target.value)} fullWidth
                                />
                            </Box>
                        </Box>
                    </Box>
                    ...
                    <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                            <TextField
                                size="small" value={amenityInput} onChange={(e) => setAmenityInput(e.target.value)}
                                placeholder="Enter an amenity" fullWidth
                            />
                            <Button onClick={handleAddAmenity} variant="contained" size="small" sx={{ minWidth: '100px', bgcolor: '#1e3a8a', '&:hover': { bgcolor: '#1e3a8a', opacity: 0.9 } }}>Add</Button>
                        </Box>
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                            {formData.amenities.map((item, index) => (
                                <Chip key={index} label={item} onDelete={() => handleRemoveAmenity(item)}
                    ...
                    <Box ref={courtsRef}>
                        ...
                        <Box sx={{ mb: 2 }}>
                            {formData.courts.map((subCourt, index) => (
                                <Box key={subCourt.id || index} sx={{ display: 'flex', gap: 2, mb: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
                                    <TextField
                                        required size="small" label="Court Name/Number" value={subCourt.name}
                                    ...
                                    />
                                    <TextField
                                        required size="small" label="Surface Type" value={subCourt.surface}
                                    ...
                                    />
                                    <Typography variant="caption" sx={{ alignSelf: 'center', minWidth: '100px' }}>
                                        Status: {subCourt.status}
                                    </Typography>
                                    <IconButton onClick={() => handleRemoveSubCourt(index)} disabled={formData.courts.length === 1} color="error">
                                        <DeleteIcon />
                                    </IconButton>
                                </Box>
                            ))}
                    ...
                    <Box ref={descriptionRef}>
                        ...
                        <TextField
                            required multiline rows={4} value={formData.description}
                    ...
                    <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
                            {existingImageUrls.map((url, index) => (
                                <Box key={`existing-${url}-${index}`} sx={{ position: 'relative', width: 150, height: 150, borderRadius: 1, overflow: 'hidden', border: '1px solid #ddd' }}>
                                    <img src={url} alt={`Existing Facility ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                    <IconButton onClick={() => handleDeleteExistingImage(url)} sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'rgba(0,0,0,0.5)', '&:hover': { bgcolor: 'rgba(0,0,0,0.7)' } }}>
                                        <DeleteIcon sx={{ color: 'white', fontSize: '1rem' }} />
                                    </IconButton>
                                </Box>
                            ))}
                            {newImagesToUpload.map((image, index) => (
                                <Box key={`new-${image.preview}-${index}`} sx={{ position: 'relative', width: 150, height: 150, borderRadius: 1, overflow: 'hidden', border: '1px solid #ddd' }}>
                                    ...
                                    <img src={image.preview} alt={`New Facility Preview ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                    {!isSubmitting && (
                                        <IconButton onClick={() => handleRemoveNewImage(index)} sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'rgba(0,0,0,0.5)', '&:hover': { bgcolor: 'rgba(0,0,0,0.7)' } }}>
                                            <DeleteIcon sx={{ color: 'white', fontSize: '1rem' }} />
                                        </IconButton>
                                    )}
                                </Box>
                            ...
                            {(existingImageUrls.length + newImagesToUpload.length) < 5 && !isSubmitting && (
                                <Box component="label" sx={{ width: 150, height: 150, border: '2px dashed #ccc', borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', cursor: 'pointer', '&:hover': { borderColor: '#1e3a8a', bgcolor: 'rgba(30, 58, 138, 0.04)' } }}>
                                    <input type="file" hidden accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { handleAddNewImage(file); e.target.value = ''; } }} />
                                    <AddPhotoAlternateIcon sx={{ fontSize: 30, color: '#666', mb: 1 }} />
                                    <Typography variant="caption" color="text.secondary">Add Image</Typography>
                                </Box>
                            )}
                        </Box>
                    </Box>
            ...
            <DialogActions sx={{ p: 3, bgcolor: 'grey.50' }}>
                <Button onClick={onClose} variant="outlined" disabled={isSubmitting} sx={{ mr: 1, borderColor: '#1e3a8a', color: '#1e3a8a' }}>Cancel</Button>
                <Button onClick={handleSubmit} variant="contained" disabled={isSubmitting} sx={{ px: 4, bgcolor: '#1e3a8a', '&:hover': { bgcolor: '#1e3a8a', opacity: 0.9 } }}>
